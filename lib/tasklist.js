...
async function tasklist(option = {}){
  ...
  if (!list) throw new Failure("Unexpected pattern", {code: "ERR_UNEXPECTED_TASKLIST_OUTPUT", info: {output: cmd.stdout, args: args, options: options} });
...
function parseLine(line,options){
  ...
  if (result.windowTitle === "N/A") result.windowTitle = null;
  if (result.user === "N/A") result.user = null;
